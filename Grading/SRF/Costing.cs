    ...
    public partial class Costing : Form
    {
        ...
        private void getData(DateTime tgl)
        {
            try
            {
                ...
                // Create a DataSet.
                data = new DataSet();
                //string strAdapter = "SELECT * FROM srfcosting WHERE DATE(datercvd)=DATE('" + tgl.ToString("yyyy-MM-dd") + "')";
                string strAdapter = "SELECT " +
                    ...
                    ",srfcosting.fabric " +
                    // ",if(date(srfcosting.datercvd) < date('2015-07-08') AND date(srfcosting.datercvd) <> date('0000-00-00') , 1, 0) as rcvdprev "+
                    // ",if(date(srfcosting.datesend) < date('2015-07-08') AND date(srfcosting.datesend) <> date('0000-00-00'), 1, 0) as finishprev "+
                    // ",if(date(srfcosting.datercvd) = date('2015-07-08') AND date(srfcosting.datercvd) <> date('0000-00-00') , 1, 0) as rcvdtoday "+
                    //",if(date(srfcosting.datesend) = date('2015-07-08') AND date(srfcosting.datesend) <> date('0000-00-00'), 1, 0) as finishtoday "+
                ...
                masterDataAdapter.Fill(data, "srfcosting");
                sqlCommandBuilder = new MySqlCommandBuilder(masterDataAdapter);

                masterBindingSource.DataSource = data;
                masterBindingSource.DataMember = "srfcosting";
        ...
        private bool insertBooking(string styleid, string sizeid, DateTime? datercvd, DateTime? datesend, string status, string crffrom, string fabric, string doneby, string remark)
        {
            ...
            try
            {
                ...
                //Command.CommandText = "INSERT INTO srfcosting(styleid,sizeid,datercvd,datesend,status,crffrim,fabric,doneby,remark) VALUES(@STYLEID,@MEID,@SIZEID,@DIRECTION,@BASIC)";
                Command.CommandText = "INSERT INTO srfcosting (styleid, sizeid, datercvd, datesend, status, crffrom, fabric, doneby, Remark) VALUES (@styleid, @sizeid, @datercvd, @datesend, @status, @crffrom, @fabric, @doneby, @Remark)";
                //item insert
        ...
        private bool updateBooking(string styleid1, string sizeid1, string styleid, string sizeid, DateTime? datercvd, DateTime? datesend, string status, string crffrom, string fabric, string doneby, string remark)
        {
            ...
            try
            {
                ...
                // Command.CommandText = "Select * from Orders ";
                //Command.CommandText = "INSERT INTO srfcosting(styleid,sizeid,datercvd,datesend,status,crffrim,fabric,doneby,remark) VALUES(@STYLEID,@MEID,@SIZEID,@DIRECTION,@BASIC)";
                Command.CommandText = "UPDATE srfcosting SET styleid=@styleid, sizeid=@sizeid, datercvd=@datercvd, datesend=@datesend, status=@status, crffrom=@crffrom, fabric=@fabric, doneby=@doneby, Remark=@Remark WHERE styleid=@styleid1 AND sizeid=@sizeid1";
                //key
        ...
        private bool deleteBooking(string styleid, string sizeid)
        {
            ...
            try
            {
                ...
                // Command.CommandText = "Select * from Orders ";
                //Command.CommandText = "INSERT INTO srfcosting(styleid,sizeid,datercvd,datesend,status,crffrim,fabric,doneby,remark) VALUES(@STYLEID,@MEID,@SIZEID,@DIRECTION,@BASIC)";
                Command.CommandText = "DELETE FROM srfcosting WHERE styleid=@styleid AND sizeid=@sizeid";
                //item insert
                Command.Parameters.AddWithValue("@styleid", styleid);
                Command.Parameters.AddWithValue("@sizeid", sizeid);
                Command.ExecuteNonQuery();
        ...
        private void cmdXSD_Click(object sender, EventArgs e)
        {
            ...
            dsReject.WriteXmlSchema("C:\\MyGarmentReport\\srfcostingxsd.xsd");
        }

        private void cmdPrint_Click(object sender, EventArgs e)
        {
            CostingFrm f = new CostingFrm();
