    ...
    public partial class Pattern : Form
    {
        ...
        private void getData(DateTime tgl)
        {
            try
            {
                ...
                // Create a DataSet.
                data = new DataSet();
                //string strAdapter = "SELECT * FROM srfpattern WHERE DATE(datercvd)=DATE('" + tgl.ToString("yyyy-MM-dd") + "')";
                string strAdapter = "SELECT " +
                    ...
                    ",srfpattern.fabric " +
                    // ",if(date(srfpattern.datercvd) < date('2015-07-08') AND date(srfpattern.datercvd) <> date('0000-00-00') , 1, 0) as rcvdprev "+
                    // ",if(date(srfpattern.datesend) < date('2015-07-08') AND date(srfpattern.datesend) <> date('0000-00-00'), 1, 0) as finishprev "+
                    // ",if(date(srfpattern.datercvd) = date('2015-07-08') AND date(srfpattern.datercvd) <> date('0000-00-00') , 1, 0) as rcvdtoday "+
                    //",if(date(srfpattern.datesend) = date('2015-07-08') AND date(srfpattern.datesend) <> date('0000-00-00'), 1, 0) as finishtoday "+
                ...
                masterDataAdapter.Fill(data, "srfpattern");
                sqlCommandBuilder = new MySqlCommandBuilder(masterDataAdapter);

                masterBindingSource.DataSource = data;
                masterBindingSource.DataMember = "srfpattern";
        ...
        private bool insertBooking(string styleid, string sizeid, DateTime? datercvd, DateTime? datesend, string status, string crffrom, string fabric, string doneby, string remark)
        {
            ...
            try
            {
                ...
                //Command.CommandText = "INSERT INTO srfpattern(styleid,sizeid,datercvd,datesend,status,crffrim,fabric,doneby,remark) VALUES(@STYLEID,@MEID,@SIZEID,@DIRECTION,@BASIC)";
                Command.CommandText = "INSERT INTO srfpattern (styleid, sizeid, datercvd, datesend, status, crffrom, fabric, doneby, Remark) VALUES (@styleid, @sizeid, @datercvd, @datesend, @status, @crffrom, @fabric, @doneby, @Remark)";
                //item insert
        ...
        private bool updateBooking(string styleid1, string sizeid1, string styleid, string sizeid, DateTime? datercvd, DateTime? datesend, string status, string crffrom, string fabric, string doneby, string remark)
        {
            ...
            try
            {
                ...
                // Command.CommandText = "Select * from Orders ";
                //Command.CommandText = "INSERT INTO srfpattern(styleid,sizeid,datercvd,datesend,status,crffrim,fabric,doneby,remark) VALUES(@STYLEID,@MEID,@SIZEID,@DIRECTION,@BASIC)";
                Command.CommandText = "UPDATE srfpattern SET styleid=@styleid, sizeid=@sizeid, datercvd=@datercvd, datesend=@datesend, status=@status, crffrom=@crffrom, fabric=@fabric, doneby=@doneby, Remark=@Remark WHERE styleid=@styleid1 AND sizeid=@sizeid1";
                //key
        ...
        private bool deleteBooking(string styleid, string sizeid)
        {
            ...
            try
            {
                ...
                // Command.CommandText = "Select * from Orders ";
                //Command.CommandText = "INSERT INTO srfpattern(styleid,sizeid,datercvd,datesend,status,crffrim,fabric,doneby,remark) VALUES(@STYLEID,@MEID,@SIZEID,@DIRECTION,@BASIC)";
                Command.CommandText = "DELETE FROM srfpattern WHERE styleid=@styleid AND sizeid=@sizeid";
                //item insert
                Command.Parameters.AddWithValue("@styleid", styleid);
                Command.Parameters.AddWithValue("@sizeid", sizeid);
                Command.ExecuteNonQuery();
        ...
        private void cmdXSD_Click(object sender, EventArgs e)
        {
            ...
            dsReject.WriteXmlSchema("C:\\MyGarmentReport\\srfpatternxsd.xsd");
        }

        private void cmdPrint_Click(object sender, EventArgs e)
        {
            PatternFrm f = new PatternFrm();
